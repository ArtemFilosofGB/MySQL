...
SELECT p.user_id
FROM profiles p
WHERE p.user_id NOT IN (SELECT from_user_id FROM messages);
```
...
```
SELECT u.id,u.firstname,u.lastname, count(f.status) AS "cnt"
FROM users u 
LEFT JOIN 
friend_requests f ON (u.id = f.initiator_user_id OR u.id = f.target_user_id)
...
Homework04_DB.sql
[/Homework04_DB.sql](/Homework04_DB.sql)
